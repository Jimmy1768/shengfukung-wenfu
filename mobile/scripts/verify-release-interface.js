...
for (const lane of ['testflight', 'production']) {
  ...
  if (!profile || profile.channel !== lane || profile.distribution !== 'store' || profile.env?.TEMPLEMATE_PUBLIC_API_ORIGIN !== 'https://shengfukung.com.tw') throw new Error(`Invalid ${lane} release interface.`);
