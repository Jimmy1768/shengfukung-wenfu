...
- Registrations copy the service’s `registration_period_key` into `metadata["registration_period_key"]`. Duplicate detection enforces one active registration per `(registrant_scope, service.slug, registration_period_key)` so recurring services (lanterns, tables, incense) cannot be double-booked.
