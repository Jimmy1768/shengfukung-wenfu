...
- `/frontend/assets/` is reserved for Vue's Vite bundles (served directly from `/var/www/<slug>`), while `/assets/` must route to Rails so admin and showcase pages can load Rails asset-pipeline CSS/JS. Keep these prefixes explicit in nginx (do not let `/assets/` fall through to the Vue `location /` fallback), and restage configs with `bin/stage_ops_configs` whenever you adjust either path.
...
- 2026-03-05: Fixed nginx asset split docs: Vue static assets are `/frontend/assets/*`, Rails asset pipeline routes are `/assets/*`; added explicit post-render/certbot verification guidance.
