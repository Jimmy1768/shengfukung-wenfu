...
- A tenant's `patron_checkout_provider` setting can also be `"cash_only"`, which disables online checkout entirely for that temple (`ProviderResolver.online_checkout_available?` returns `false`) rather than selecting an adapter. The demo temple's config (`rails/db/temples/shengfukung-demo.yml`) sets it to `cash_only`, and production matches (checked 2026-09-26).
