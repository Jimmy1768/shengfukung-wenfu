...
- Registrations lock once fulfilled or past the start time. There is no cancel or refund action in the account area: `Payments::RefundService` exists but nothing calls it, and the ECPay adapter's refund raises `NotImplementedError`.
...
- **Payments** (`/account/payments`) lists the patron's payments, and the dashboard shows the latest three. There are no receipts.
