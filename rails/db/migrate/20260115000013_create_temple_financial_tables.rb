...
class CreateTempleFinancialTables < ActiveRecord::Migration[7.1]
  def change
    ...
    create_table :offering_registrations do |t|
      t.references :temple, null: false, foreign_key: true
      t.references :offering, polymorphic: true, null: false
    ...
    add_index :offering_registrations, [:temple_id, :reference_code], unique: true, name: "idx_offering_registrations_on_code"
    add_index :offering_registrations, [:temple_id, :payment_status], name: "idx_offering_registrations_on_payment_status"
    add_index :offering_registrations, [:temple_id, :fulfillment_status], name: "idx_offering_registrations_on_fulfillment_status"

    create_table :temple_payments do |t|
      t.references :temple, null: false, foreign_key: true
      t.references :offering_registration, null: false, foreign_key: true, index: { name: "idx_payments_on_registration" }
      t.references :user, foreign_key: true
